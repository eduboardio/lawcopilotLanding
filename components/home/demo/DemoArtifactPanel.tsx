...
import { Scale, CheckCircle, FileText, BookOpen, ChevronDown } from "lucide-react"
  ...
  <div className="h-full flex flex-col bg-background">
    ...
    <div className="h-16 px-6 border-b border-border/60 flex items-center justify-between shrink-0 bg-background/95 backdrop-blur-sm">
      ...
      <motion.span
        ...
        className="text-xs px-3 py-1.5 rounded-lg bg-green-500/20 text-green-400 flex items-center gap-1.5 font-bold border border-green-500/40"
      >
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <h3 className="text-base font-bold mb-2">Order VII Rule 11(a) CPC Analysis</h3>
      ...
      <motion.div
        ...
        transition={{ delay: 0.2 }}
        className="bg-card border-2 border-border/60 rounded-lg p-5 space-y-3 shadow-md"
      >
      ...
      >
        {[
          ...
          <motion.div
            ...
            className="bg-card border-2 border-border/60 rounded-lg p-4 text-center hover:border-primary/40 transition-colors shadow-md"
          >
      ...
      <div className="space-y-3">
        ...
        <motion.div
          ...
          animate={{
            opacity: 1,
            y: 0,
            backgroundColor: selectedCase ? "hsl(var(--primary) / 0.10)" : "hsl(var(--muted) / 0.2)",
            borderColor: selectedCase ? "hsl(var(--primary) / 0.5)" : "hsl(var(--border) / 0.6)",
          }}
          transition={{
            opacity: { delay: 0.7 },
            y: { delay: 0.7 },
          ...
          className="border-2 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow"
        >
          <div className="p-4 cursor-pointer">
            <div className="flex items-start gap-3">
              <FileText className="w-5 h-5 mt-0.5 shrink-0 text-primary" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-foreground mb-1">Mr. Manish Goel vs Mr. Raghav Goyal & Anr.</p>
          ...
          <AnimatePresence>
            {selectedCase && (
              ...
              >
                <div className="p-4 space-y-4 bg-card/70">
                  ...
                  >
                    <button className="text-xs px-4 py-2 bg-primary text-primary-foreground rounded-lg font-bold hover:bg-primary/90 transition-colors shadow-md">
                      Get Court Copy
                    </button>
                    <button className="text-xs px-4 py-2 bg-card border-2 border-border/60 rounded-lg font-bold hover:bg-muted/50 transition-colors">
                      Add to Chat
                    </button>
                    <button className="text-xs px-4 py-2 bg-card border-2 border-border/60 rounded-lg font-bold hover:bg-muted/50 transition-colors">
                      Get Cited Acts
        ...
        {[
          ...
          <motion.div
            ...
            className="border-2 border-border/40 rounded-lg p-4 opacity-70 hover:opacity-100 hover:border-border/60 transition-all cursor-pointer bg-card/50 hover:bg-card/70 shadow-sm"
          >
            <div className="flex items-start gap-3">
              <FileText className="w-5 h-5 mt-0.5 text-primary" />
              <div className="flex-1">
                <p className="text-sm font-bold text-foreground mb-1">{caseItem.name}</p>
