...
export const DemoDetailPanel = memo(() => (
  <div className="h-full flex flex-col bg-background">
    ...
    <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-5 lg:p-6 space-y-4 sm:space-y-5 md:space-y-6">
      ...
      >
        <h3 className="text-xs sm:text-sm md:text-base font-bold mb-3 sm:mb-4 text-foreground">Mr. Manish Goel vs Mr. Raghav Goyal & Anr.</h3>
      ...
      <div className="space-y-3 sm:space-y-4 md:space-y-5 text-xs sm:text-sm">
        ...
        <DocumentSection
          ...
          content={
            <div className="space-y-2 sm:space-y-3">
              <div>
                <p className="font-bold text-foreground mb-0.5 sm:mb-1">Petitioner:</p>
                <p className="text-muted-foreground leading-relaxed">Mr. Manish Goel (defendant in original suit)</p>
              </div>
              <div>
                <p className="font-bold text-foreground mb-0.5 sm:mb-1">Respondents:</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}>
          <h4 className="font-bold text-xs sm:text-sm mb-2 sm:mb-3 flex items-center gap-1.5 sm:gap-2 text-foreground">
            <div className="w-1 sm:w-1.5 h-4 sm:h-5 bg-primary rounded-full" />
            Acts & Laws Involved
          </h4>
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {["Code of Civil Procedure, 1908", "Order VII Rule 11", "Section 115"].map((act, i) => (
              <motion.span
                ...
                className="px-2 py-1 sm:px-2.5 sm:py-1.5 md:px-3 md:py-2 bg-primary/10 text-primary rounded-md sm:rounded-lg text-[10px] sm:text-xs font-bold border border-primary/30 hover:bg-primary/20 transition-colors shadow-sm"
              >
                {act}
              </motion.span>
            ))}
...
const DocumentSection = memo(
  ({
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}>
      ...
      <div className="text-muted-foreground leading-relaxed pl-3 sm:pl-4">
